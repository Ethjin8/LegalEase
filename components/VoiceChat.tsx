...
export default function VoiceChat({ documentId, voiceMessages, voiceState, voiceError, language }: Props) {
  ...
  // Merge voice transcripts + text messages, sorted by timestamp
  const allMessages = [...voiceMessages, ...textMessages].sort(
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setTextMessages((prev) => [...prev, { role: "model", text: answer, timestamp: new Date() }]);
    } catch {
      setTextMessages((prev) => [...prev, { role: "model", text: "Connection error. Please try again.", timestamp: new Date() }]);
    } finally {
      setLoading(false);
    ...
    >
      ...
      <div style={{
        ...
        {voiceState !== "idle" && (
          <span style={{
            fontSize: "0.68rem",
            color: voiceState === "listening" ? "#ef4444" : "#2563eb",
            fontWeight: 600,
            ...
            {voiceState === "listening" ? "Live" : voiceState === "speaking" ? "Speaking" : voiceState === "connecting" ? "Connecting" : "Thinking"}
          </span>
        )}
      </div>
        ...
        {allMessages.length === 0 && (
          <div style={{ textAlign: "center", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
            ...
            <p style={{ color: "#9ca3af", fontSize: "0.88rem", fontFamily: "'Inter', sans-serif", maxWidth: 220, lineHeight: 1.6 }}>
              Ask anything about this document — by voice or text.
            </p>
          </div>
        )}
